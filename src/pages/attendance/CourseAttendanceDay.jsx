...
import { getGroupedCourseDayAttendance, overrideStudentAttendance } from '../../api/attendance';
...
const CourseAttendanceDay = () => {
  ...
  const loadDay = async () => {
    ...
    setError('');
    try {
      const response = await getGroupedCourseDayAttendance(courseKey, date);
      setData(unwrap(response));
    } catch (err) {
      setError(getApiErrorMessage(err, 'Unable to load course day attendance'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const studentRows = useMemo(() => {
    const rows = asArray(data?.studentAttendance || data?.students || data?.records);
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-4">
        <button
          type="button"
          onClick={() => navigate(`/courses/${encodeURIComponent(courseKey)}/attendance`)}
